...
mod tests {
    ...
    use k8s_openapi::serde_json::ser::State;
    use kube::api::{DeleteParams, Patch, PostParams};
    use kube::config::{Context, KubeConfigOptions};
    use kube::runtime::controller::Action;
    ...
    async fn integration_reconcile_should_generate_secret_with_regeneration() {
        ...
        let username_after_cron =
            from_utf8(&*secret_after_cron.get("username").unwrap().0).unwrap();
