...
async fn main() -> anyhow::Result<()> {
    ...
    if args.requeue_duration == 0 {
        return Err(anyhow!("requeue-duration must be greater than 0"));
    }
    ...
    let is_one_shot = args.one_shot || args.mode == "one-shot";
    if is_one_shot {
        info!("Running runo in one-shot mode.");
        reconciler::run_one_shot(config).await?;
        Ok(())
    } else if args.mode == "reconciliation" {
        ...
        Err(anyhow!("Mode is not supported!: {:?}", args.mode))
    }
}
